...
import {bootstrapOwner} from './config';
import {type State,type Loan,type LoanItem,type Event,defaultSettings} from '../app/domain';
...
function omit<T extends object,K extends keyof T>(value:T,keys:readonly K[]):Omit<T,K>{const copy={...value};for(const key of keys)delete copy[key];return copy}
const fromRows=<T>(rows:Row[])=>rows.map(r=>JSON.parse(r.payload) as T);
export class Repository{
 constructor(public db:SqlDatabase,private ownerIdentity=bootstrapOwner()){}
 async actor(user:User):Promise<Actor>{
  ...
  if(!workspace){
   if(!this.ownerIdentity.id||!this.ownerIdentity.email||user.userId!==this.ownerIdentity.id||user.email.toLowerCase()!==this.ownerIdentity.email)throw new AppError('A assistência ainda precisa ser configurada pela proprietária.',403);
 ...
 async read(actor:Actor):Promise<Snapshot>{
  const sql=[
   'SELECT revision,settings,owner_id FROM workspaces WHERE id=?',
   'SELECT payload FROM pieces WHERE workspace_id=? ORDER BY sku',
   'SELECT payload,id,portal_enabled,portal_user_id,email FROM merchants WHERE workspace_id=? ORDER BY name_key',
   ...
   'SELECT payload FROM receivables WHERE workspace_id=? ORDER BY created_at DESC,id DESC',
   'SELECT payload FROM notices WHERE workspace_id=? ORDER BY at DESC,id DESC',
   'SELECT payload FROM movements WHERE workspace_id=? ORDER BY at DESC,id DESC',
   'SELECT payload FROM payments WHERE workspace_id=? ORDER BY created_at DESC,id DESC'
  ];
  const r=await this.db.batch(sql.map(q=>this.db.prepare(q).bind(WORKSPACE)));
  const w=r[0].results?.[0] as {revision:number;settings:string;owner_id:string}|undefined;if(!w)throw new AppError('Assistência não encontrada.',404);
  if(actor.role==='owner'){if(w.owner_id!==actor.id)throw new AppError('Seu acesso à assistência foi revogado.',403)}else{const m=r[2].results.find(row=>row.id===actor.merchantId);if(!m||m.portal_enabled!==1||m.portal_user_id!==actor.id||m.email!==actor.email.toLowerCase())throw new AppError('Seu acesso ao portal foi desabilitado. Fale com a assistência.',403)}
 ...
 async execute(actor:Actor,id:string,revision:number,input:unknown):Promise<Snapshot&{result:Record<string,string|number|boolean>}>{
  ...
  const diff=<T extends {id:string}>(old:T[],now:T[],write:(item:T)=>SqlStatement)=>{const map=new Map(old.map(x=>[x.id,j(x)]));for(const item of now)if(map.get(item.id)!==j(item))statements.push(write(item))};
  ...
  const loanHeader=(l:Loan)=>omit(l,['items','events']);
  ...
  diff(before.state.receivables,next.receivables,r=>this.db.prepare('INSERT INTO receivables (id,workspace_id,loan_id,merchant_id,amount,paid_amount,payload,created_at) VALUES (?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET paid_amount=excluded.paid_amount,payload=excluded.payload').bind(r.id,WORKSPACE,r.loanId,r.merchantId,r.amount,r.paidAmount??(r.paid?r.amount:0),j(r),r.created));
  const oldPayments=new Set((before.state.payments||[]).map(p=>p.id));for(const p of next.payments||[])if(!oldPayments.has(p.id))statements.push(this.db.prepare('INSERT INTO payments (id,workspace_id,receivable_id,amount,payload,created_at) VALUES (?,?,?,?,?,?)').bind(p.id,WORKSPACE,p.receivableId,p.amount,j(p),p.at));
