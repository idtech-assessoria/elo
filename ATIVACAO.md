...
A proprietária confirmou `idtech.assessoria@gmail.com` pelo e-mail do Supabase em 11/09/2026 às 03:11:15 UTC. A assistência original foi restaurada e vinculada ao UUID real dessa conta; o contato foi atualizado para o e-mail autorizado e a data/configurações originais foram preservadas. `ELO_OWNER_USER_ID` foi instalado no Render somente depois da restauração. O login com senha e a ativação por código estão **live às 04:26:57 UTC**, no deploy `dep-daho58id0e5s7387g2gg`, commit `86927b9e1118a0ddd78283453811ea8e3afb23be`, após CI completo e preflight PostgreSQL. As telas públicas foram conferidas no navegador. Após autorização específica, um único código foi emitido às 04:58:30 UTC e a função temporária foi desativada às 04:58:39 UTC, com HTTP 410 verificado. A titular escolhe sua senha no Elo; nenhum e-mail foi enviado nem senha real alterada pelo assistente. O registro sem credenciais está em [docs/PRIMEIRA-SENHA.md](docs/PRIMEIRA-SENHA.md). O acesso administrativo fica na raiz `https://elo-validacao.onrender.com/`; o canal de avisos operacionais ainda não está configurado.
