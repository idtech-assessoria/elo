...
A proprietária confirmou `idtech.assessoria@gmail.com` pelo e-mail do Supabase em 11/09/2026 às 03:11:15 UTC. A captura enviada mostra o Elo aberto após o retorno do login. A assistência original foi restaurada e vinculada ao UUID real dessa conta; o contato foi atualizado para o e-mail autorizado e a data/configurações originais foram preservadas. `ELO_OWNER_USER_ID` foi instalado no Render somente depois da restauração. O deploy `dep-dahn5uafngtc73d7if40`, commit `10135336a24efb3cf9df5305c7653efaab3f159d`, ficou **live às 03:20:04 UTC**, após verificar a conexão PostgreSQL, o papel restrito, as 14 tabelas com RLS e as permissões de sessão. O acesso administrativo deve ser feito pela raiz `https://elo-validacao.onrender.com/`; o canal de avisos operacionais ainda não está configurado.
...
O próximo passo é abrir o [painel da assistência](https://elo-validacao.onrender.com/) no navegador em que a proprietária confirmou o e-mail. A captura anterior mostrava a mensagem de assistência não configurada; a ausência do registro e do vínculo foi corrigida pela restauração. A leitura do painel após essa correção ainda precisa ser confirmada pelo titular. Não solicitar outro e-mail automaticamente.
