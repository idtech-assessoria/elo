...
Resend foi consultado e ainda contém `idtech.com.br` com verificação `failed`. O titular solicitou excluir esse cadastro, criado por engano. A revisão automática bloqueou a remoção até uma confirmação após o aviso de que a exclusão é irreversível e interrompe envios/recebimentos do domínio no Resend. A exclusão não aconteceu. O domínio foi descartado das instruções de ativação e nenhum DNS foi alterado. A base continua com zero conexões Resend/Gmail, zero contas Auth da proprietária e zero assistências.
