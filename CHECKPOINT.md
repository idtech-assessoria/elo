...
Estado mais recente: a correção do retorno ao login está publicada no deploy `dep-dahnfv7qj5pc739olra0`, commit `ee2f2481f246eb6290918ecdad425b8968096761`, **live às 03:41:17 UTC de 11/09/2026**. O CI completo e a conexão/permissões PostgreSQL passaram. A tela pública “Entrar no Elo” foi verificada no navegador em `https://elo-validacao.onrender.com/login`, assim como o retorno a ela ao revisitar `/auth/logout`. O ciclo de sair e entrar novamente foi testado com Auth local simulado, sem e-mails reais. A assistência original continua vinculada à conta confirmada de `idtech.assessoria@gmail.com`; canais de avisos operacionais seguem pendentes. Não repetir a importação, criar outro servidor ou regenerar segredos.
