...
Estado mais recente: o login com e-mail e senha e a ativação por código estão publicados no deploy `dep-daho58id0e5s7387g2gg`, commit `86927b9e1118a0ddd78283453811ea8e3afb23be`, **live às 04:26:57 UTC de 11/09/2026**, após CI completo. As duas telas foram conferidas no navegador e o preflight PostgreSQL passou. A emissão do código para a primeira senha foi bloqueada pela revisão automática e exige autorização específica; nenhum código foi emitido nem senha real alterada. O ciclo de sair e entrar novamente com senha foi testado com Auth local simulado. A assistência original continua vinculada à conta confirmada de `idtech.assessoria@gmail.com`; canais de avisos operacionais seguem pendentes. Não repetir a importação, criar outro servidor ou regenerar segredos.
...
- O [CI da ativação](https://github.com/idtech-assessoria/elo/actions/runs/34562002836) passou em todas as etapas, incluindo PostgreSQL 17, concorrência, lint, TypeScript, build e HTTP. O deploy `dep-daho58id0e5s7387g2gg` ficou live às 04:26:57 UTC. O log às 04:26:49 UTC confirmou conexão, papel restrito, 14 tabelas com RLS e função privada de sessão. O navegador mostrou `/login` com e-mail/senha e `/password-setup` com e-mail/código; nenhum formulário real foi submetido pelo assistente.
- A revisão automática rejeitou a implantação da função temporária que emitiria o código: privilégios `service_role` para recuperação real de conta exigem autorização específica e ampliam a superfície de autenticação. A listagem posterior confirmou zero Edge Functions; nenhum código foi emitido. O material da capacidade administrativa não utilizada foi removido, sem tentativa de contornar a rejeição. A operação proposta e seu modelo estão em [docs/PRIMEIRA-SENHA.md](docs/PRIMEIRA-SENHA.md). A titular ainda precisa escolher a senha e verificar sua entrada real; nunca incluir códigos, senhas ou capacidades administrativas neste checkpoint.
