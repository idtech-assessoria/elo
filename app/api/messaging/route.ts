...
export async function GET(){try{const {repository,actor}=await context();return json(await new Messaging(repository.db,env.MESSAGING_ENCRYPTION_KEY).snapshot(actor))}catch(e){return errorResponse(e)}}
export async function POST(request:Request){try{
 ...
 const {repository,actor}=await context();if(actor.role!=='owner')throw new AppError('Somente a administradora pode gerenciar os envios.',403);const m=new Messaging(repository.db,env.MESSAGING_ENCRYPTION_KEY);const input=v.data;
