...
export function useWorkspace(){
 ...
 const refresh=useCallback((quiet=false,force=false)=>{if(running.current||editLock.current&&!force)return Promise.resolve();return fetchJson<Envelope>('/api/workspace').then(v=>{if(!editLock.current||force)accept(v)}).catch(e=>{if(e instanceof ApiError&&e.status===403){router.replace('/portal');return}if(!quiet||e instanceof ApiError&&e.status===401)setError((e as Error).message);setLoading(false)})},[accept,router]);
