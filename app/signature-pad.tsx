...
export default function SignaturePad({ strokes, onChange, disabled, onLimit }: { strokes: SignatureStrokes; onChange: (value: SignatureStrokes) => void; disabled: boolean; onLimit: () => void }) {
  ...
  const point = (event: PointerEvent<SVGSVGElement>): SignaturePoint => {
  ...
  return <div className="signature-pad">
    <svg viewBox="0 0 1000 400" preserveAspectRatio="none" role="img" aria-label="Espaço para o lojista assinar com o dedo ou mouse" className={disabled ? 'disabled' : ''}
      onPointerDown={event => { if (disabled || event.button !== 0) return; if (strokes.length >= 50) { onLimit(); return; } event.preventDefault(); active.current = event.pointerId; event.currentTarget.setPointerCapture(event.pointerId); publish([...strokes, [point(event)]]); }}
      onPointerMove={event => { if (disabled || active.current !== event.pointerId) return; event.preventDefault(); const current = drawing.current; const last = current.at(-1)!; if (last.length >= 500 || current.reduce((sum, stroke) => sum + stroke.length, 0) >= 2500) { active.current = null; onLimit(); return; } const nextPoint = point(event); const previous = last.at(-1)!; if (Math.hypot(previous[0] - nextPoint[0], previous[1] - nextPoint[1]) < 2) return; publish([...current.slice(0, -1), [...last, nextPoint]]); }}
