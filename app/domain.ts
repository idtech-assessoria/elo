import type { SignedReceipt } from './receipt-model';
...
export type Loan = { id:string; merchantId:string; created:string; due:string; acknowledged:boolean; items:LoanItem[]; notes:string; events:Event[]; acknowledgedBy?:string; acknowledgedAt?:string; dispute?:string; requests?:ExtensionRequest[]; receipt?:SignedReceipt };
...
export const loanStatus = (l:Loan)=>!loanCount(l)?'Concluído':l.due<today()?'Atrasado':l.items.some(i=>i.returned+i.sold>0)?'Devolução parcial':'Em andamento';
...
export function createLoan(s:State,merchantId:string,due:string,quantities:Record<string,number>,notes:string):{state:State;loan:Loan}{
 ...
 const l:Loan={id:'EMP-'+String(Math.max(0,...s.loans.map(l=>Number(l.id.split('-')[1])||0))+1).padStart(4,'0'),merchantId,due,created:today(),items,notes:notes.trim(),acknowledged:false,events:[{id:uid('evento'),title:'Empréstimo registrado',description:'Saída do estoque conferida pela assistência. Comprovante disponível para assinatura.',at:stamp()}]};
 const next={...s,pieces:s.pieces.map(p=>({...p,available:p.available-(quantities[p.id]||0)})),loans:[l,...s.loans],notices:[...makeNoticePair(s,l,'Novo empréstimo registrado','Consulte as peças emprestadas e o prazo combinado. A assinatura é coletada no comprovante pela assistência.'),...s.notices],movements:[{id:uid('mov'),title:'Empréstimo registrado',description:`${m.name} · ${loanCount(l)} peças · ${money(total)}`,at:stamp()},...s.movements]};return {state:next,loan:l};
}
export function settleItem(s:State,loanId:string,productId:string,quantity:number,action:'return'|'quarantine'|'sale',saleUnitValue?:number):State{
 ...
 const isSale=action==='sale';const unitValue=i.unitValue>0?i.unitValue:saleUnitValue; if(isSale&&(typeof unitValue!=='number'||!Number.isSafeInteger(unitValue)||unitValue<1||unitValue>100000000))throw Error('Informe o valor unitário da venda para esta peça sem valor cadastrado.');const saleAmount=(unitValue||0)*quantity;const title=isSale?'Peça convertida em venda':action==='quarantine'?'Devolução em quarentena':'Devolução conferida';
 ...
 const next={...s,loans:s.loans.map(x=>x.id===loanId?nextLoan:x),pieces:s.pieces.map(p=>p.id===productId?{...p,available:p.available+(action==='return'?quantity:0),quarantine:p.quarantine+(action==='quarantine'?quantity:0)}:p),receivables:isSale?[{id:uid('receber'),loanId,merchantId:l.merchantId,amount:saleAmount,paid:false,paidAmount:0,description:`${quantity} × ${i.name}`,created:stamp()},...s.receivables]:s.receivables,notices:[...makeNoticePair(s,nextLoan,title,`${quantity} × ${i.name}. ${isSale?'Pagamento pendente: '+money(saleAmount):'Devolução registrada.'}`),...s.notices],movements:[{id:uid('mov'),title,description:`${quantity} × ${i.name} · ${l.id}${isSale?' · '+money(saleAmount):''}`,at:stamp()},...s.movements]};return next;
}
