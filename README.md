...
O login em `/login` usa e-mail e senha, verificados pelo Supabase Auth, sem solicitar e-mail a cada entrada. A página `/account/password` permite definir ou alterar a senha somente da conta autenticada, com sessão válida e não revogada. Para uma conta previamente confirmada que ainda dependa do link por e-mail, `/password-setup` aceita um código de recuperação nativo do Supabase fornecido pelo administrador autorizado; o código é temporário, de uso único e enviado somente no corpo do POST. A nova senha é escolhida pelo próprio titular depois da autenticação. A aplicação não cria códigos nem possui chave administrativa de Auth. O link mágico com PKCE continua como alternativa opcional e deve ser aberto no mesmo navegador.
...
| `npm run test:http` | Rotas do build, senha, ativação por código, PKCE/cookies/CSRF e saída/reentrada; Auth local simulado |
...
Veja [MIGRACAO.md](MIGRACAO.md), [ATIVACAO.md](ATIVACAO.md) e [CHECKPOINT.md](CHECKPOINT.md) para a origem verificada e os requisitos de ativação. `render.yaml` registra a configuração da hospedagem de validação já criada no Render Free.
